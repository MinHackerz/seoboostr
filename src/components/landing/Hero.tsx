      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-base sm:text-lg text-slate-500 max-w-xl mx-auto leading-relaxed">
            10 audit modules run simultaneously. You get one clean, scored report.
        ...
        {isLoggedIn ? (
          <motion.div
            ...
            className="flex flex-col items-center gap-4 max-w-md mx-auto mb-8 sm:mb-10"
          >
            <a
              href="/dashboard"
              className="w-full flex items-center justify-center gap-2 h-12 px-6 rounded-xl text-sm font-bold text-white bg-teal-600 hover:bg-teal-700 hover:shadow-lg transition-all duration-200 hover:-translate-y-0.5 cursor-pointer shadow-md select-none"
            >
            ...
            <p className="text-xs text-slate-400 font-medium select-none">
              Logged in as <span className="font-semibold text-slate-500">{session?.user?.email}</span>
            </p>
          </motion.div>
        ) : (
          <motion.form
